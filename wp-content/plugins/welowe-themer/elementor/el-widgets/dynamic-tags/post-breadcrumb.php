...
class GVAElement_Post_Breadcrumb extends GVAElement_Base{
	...
	public function get_title(){
		return esc_html__('Breadcrumb', 'welowe-themer');
	}
	...
	protected function register_controls(){
		...
		$this->add_control(
			'bg_overlay_color',
			[
				'label' => esc_html__( 'Background Overlay Color', 'welowe-themer' ),
		...
		$this->add_control(
	  	...
	  	$this->add_responsive_control(
			'padding_top',
			[
				'label' => __( 'Breadcrumb Padding Top', 'welowe-themer' ),
		...
		$this->add_responsive_control(
			'padding_bottom',
			[
				'label' => __( 'Breadcrumb Padding Bottom', 'welowe-themer' ),
				'type' => Controls_Manager::SLIDER,
				'range' => [
					'px' => [
						'min' => 0,
		...
		$this->add_control(
			'heading_content',
			[
				'label'     => __('Content & Style ------', 'welowe-themer'),
				'type'      => Controls_Manager::HEADING,
			]
	  	);
		$this->add_control(
			'show_title',
			[
				'label'     => __('Show Titlte', 'welowe-themer'),
		...
		$this->add_control(
			'title_color',
			[
				'label' => esc_html__( 'Title Color', 'welowe-themer' ),
				'type' => Controls_Manager::COLOR,
				'selectors' => [
		...
		$this->add_control(
			'show_links',
			[
				'label'     => __('Show Breadcrumb Links', 'welowe-themer'),
				'type'      => Controls_Manager::SWITCHER,
				'default'   => 'yes',
	...
	public function breadcrumbs(){
	 	$delimiter = '';
	 	$home = esc_html__('Home', 'welowe-themer');
	 	...
	 	if(!is_home() && !is_front_page() || is_paged()) {
			...
			if(is_category()){
			  	...
			  	$breadcrumb .= $before . esc_html__('Search results for', 'welowe-themer') . '"' . htmlentities(get_search_query()) . '"' . $after;
			  ...
			  $breadcrumb .= $before . esc_html__('Posts tagged', 'welowe-themer') . '"' . single_tag_title('', false) . '"' . $after;
			  	...
			  	if($userdata){
				 	$breadcrumb .= $before . esc_html__('Articles posted by', 'welowe-themer') . $userdata->display_name . $after;
			  	} 

			}elseif(is_404()){
			  	$breadcrumb .= $before . esc_html__('Error 404', 'welowe-themer') . $after;
			}

			if(get_query_var('paged')){
			  	$breadcrumb .= $before . esc_html__('Page','welowe-themer') . ' ' . get_query_var('paged') . $after;
			}
