			 		...
			 		if( $settings['title_text'] || $settings['description_text'] ){
			 			echo '<div class="about-one__content-inner">';
			 				if($settings['title_text']){
			 					echo '<h3 class="about-one__title">' . $settings['title_text'] . '</h3>';
			 				}
			 				if($settings['description_text']){
			 					echo '<div class="about-one__desc">' . $settings['description_text'] . '</div>';
			 				}
			 				if($settings['link']['url']){
               echo '<div class="about-one__action">';
                  $this->gva_render_link_html_2(esc_html__('View Map', 'welowe-themer'), $settings['link'], 'btn-inline'); 
               echo '</div>';
              }
			 			echo '</div>';
			 		}
		  		...
		  		if( !empty($settings['image']['url']) ){
				 	echo '<div class="about-two__image">';
				 		...
				 		if(!empty($settings['image_logo']['url'])){
				 			echo '<div class="about-two__shape">';
				 				echo '<img src="'.esc_url($settings['image_logo']['url']).'" alt="'. esc_attr($settings['title_text']) . '" />';
				 			echo '</div>';
				 		}
				 		if(!empty($settings['image_shape_2']['url'])){
				 			echo '<div class="about-two__shape-2">';
				 				echo '<img src="'.esc_url($settings['image_shape_2']['url']).'" alt="'. esc_attr($settings['title_text']) . '" />';
				 			echo '</div>';
				 		}
				 	echo '</div>';
		  		} 
		  		...
		  		if( $settings['title_text']){
		 			echo '<div class="about-two__box-content">';
		 				echo '<div class="about-two__box-content-inner">';
		 					if($settings['title_text']){
			 					echo '<div class="about-two__title">' . ($settings['title_text']) . '</div>';
			 				}
			 			echo '</div>';	
		 			echo '</div>';
		 		}
	...
	<?php if($skin == 'skin-v3'){ ?>
		<div class="about-three__single">
		  	<?php 
			 	...
			 	echo '<div class="about-three__line"></div>';
			?>
		</div>
	<?php } ?> 
