...
class GVA_Layout_Action extends GVA_Layout_Model{
   ...
   public function ajax_auth_init(){ 
      add_action( 'wp_ajax_welowe_add_template_layout', array($this, 'add_template_layout') );
      add_action( 'wp_ajax_welowe_duplicate_template_layout', array($this, 'duplicate_template_layout') );
      add_action( 'wp_ajax_welowe_set_header_footer_layout', array($this, 'set_header_footer_layout') );
      add_action( 'wp_ajax_welowe_set_state_config', array($this, 'set_state_config') );
      add_action( 'wp_ajax_welowe_delete_template_layout', array($this, 'delete_template_layout') );
   }

   public function add_template_layout(){
      check_ajax_referer('welowe_ajax_security_nonce', 'security');
      ...
      echo json_encode(
         array(
            'status'  => 'success',
            'message' => esc_html__( $name . " created", 'welowe-themer')
         )
      );
      die();
   }
   ...
   public function set_header_footer_layout(){
      ...
      echo json_encode(
         array(
            'status'  => 'success',
            'message' => esc_html__( "Changes saved successfully", 'welowe-themer')
         )
      );
      die();
   }
   ...
   public function set_state_config(){
      ...
      if( empty($_POST['id']) || empty($_POST['type']) ){ 
         echo json_encode(
            array(
               'status'  => 'success',
               'message' => esc_html__( "Changes Unsuccess", 'welowe-themer')
            )
         );
         return;
      }
   ...
   public function duplicate_template_layout(){
      ...
      if (is_wp_error($new_post_id)) {
         echo json_encode(
            array(
               'status'  => 'success',
               'message' => esc_html__( "Can't Duplicate", 'welowe-themer')
            )
         );
         die();
      }
      ...
      echo json_encode(
         array(
            'status'  => 'success',
            'message' => esc_html__( "Duplicate success", 'welowe-themer')
         )
      );
      die();
   ...
   function delete_template_layout(){
      ...
      if($template_type){
         ...
         if( count($get_templates) < 2){
            echo json_encode(
               array(
                  'status'  => 'unsuccess',
                  'message' => esc_html__( "Can't delete when have only one item", 'welowe-themer')
               )
            );
            die();
         }
      ...
      global $wpdb;
      $meta_key = 'welowe_template_layout';
      ...
      if( $count->THE_COUNT > 0 ){
         echo json_encode(
            array(
               'status'  => 'unsuccess',
               'message' => esc_html__( "Can't delete, some content use this template layout!", 'welowe-themer')
            )
         );
         die();
      }
      ...
      if (!$post) {
         echo json_encode(
            array(
               'status'  => 'unsuccess',
               'message' => esc_html__( "Can't Delete Template", 'welowe-themer')
            )
         );
         die();
      }
      ...
      echo json_encode(
         array(
            'status'  => 'success',
            'message' => esc_html__( "Template deleted", 'welowe-themer')
         )
      );
      die();
