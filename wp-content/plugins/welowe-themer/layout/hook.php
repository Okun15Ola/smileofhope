...
class GVA_Layout_Frontend_Hook extends GVA_Layout_Model{
   ...
   function __construct(){
      add_action('welowe/layouts/page', array($this, 'layout_page'));
      add_action('welowe/layouts/single/post', array($this, 'layout_single_post'));
      add_action('welowe/layouts/archive/post', array($this, 'layout_archive_post'));

      add_action('welowe/layouts/single/template', array($this, 'single_template'));

      add_action('welowe/layouts/single/product', array($this, 'layout_single_product'));
      add_action('welowe/layouts/archive/product', array($this, 'layout_archive_product'));

      add_action('welowe/layouts/single/give_form', array($this, 'layout_single_give_form'));
      add_action('welowe/layouts/archive/give_forms', array($this, 'layout_archive_give_forms'));
      
      add_action('wp_enqueue_scripts', array($this, 'welowe_themer_elementor_load_css'), 500);
   }

   public function welowe_themer_elementor_load_css(){
      ...
      if($post && $post->post_type == 'give_forms'){
         ...
         $template_id = $this->get_template_default('givewp_layout');
         $post_meta_template = get_post_meta($post->ID, 'welowe_template_layout', true);
   ...
   public function layout_page(){
      ...
      if($post_meta_template == '_without_layout' || empty($post_meta_template) || !metadata_exists('post', $post_id, 'welowe_template_layout')){
   ...
   public function layout_single_product(){
      ...
      $post_id = $post->ID;
      $template_id = $this->get_template_default('single_product_layout');
      $post_template_id = get_post_meta($post_id, 'constix_template_layout', true);
   ...
   public function layout_archive_product(){
      ...
      do_action('constix_page_breacrumb');
   ...
   public function layout_single_give_form(){
      ...
      $post_id = $post->ID;
      $template_id = $this->get_template_default('givewp_layout');
      $post_template_id = get_post_meta($post_id, 'welowe_template_layout', true);
      if( !empty($post_template_id) && $post_template_id && $post_template_id != '_default_active' && is_numeric($post_template_id) ){
         $template_id = $post_template_id;
      }
      if($template_id){
   ...
   public function layout_archive_give_forms(){
      global $post;
      $page_template = $this->get_template_default('givewp_archive_layout');
