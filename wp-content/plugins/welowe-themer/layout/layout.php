...
class GVA_Template_Layout extends GVA_Layout_Model{
   ...
   function __construct(){
	...
	public function add_admin_menu(){
		add_menu_page(
			esc_html__('Welowe Template', 'welowe-themer'),
			esc_html__('Welowe Template', 'welowe-themer'),
			'manage_options',
			'welowe_layout_template',
   ...
   public function get_checkboxs_header_footer($template_type = 'header_layout', $layout_id = 0, $value=''){
	...
	public function show_options(){
      wp_enqueue_style('admin-template-layout', GAVIAS_WELOWE_PLUGIN_URL . 'layout/assets/admin.css'); 
      wp_register_script('admin-template-layout', GAVIAS_WELOWE_PLUGIN_URL . 'layout/assets/admin.js', array('jquery') ); 
      ...
      // $config->get_post_config(false);
		...
		wp_localize_script( 'admin-template-layout', 'form_ajax_object', array( 
		  ...
		  'security_nonce' => wp_create_nonce( "welowe_ajax_security_nonce" )
		));
