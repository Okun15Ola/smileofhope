<?php
  ...
  if($goal == 'unlimited'){
    $progress_label = esc_html__( 'unlimited' , 'welowe' );
    $progress = 100;
  }else{
  ...
  if(!isset($excerpt_words)){
    $excerpt_words = welowe_get_option('give_excerpt_limit', 10);
  }
?> 
<div  class="give-block">
  <div <?php post_class(); ?>>
    <div class="form-image">
      ...
      <?php } ?>
      <div class="content-action"><a class="link" href="<?php esc_url(the_permalink()) ?>"><?php echo esc_html__( 'Donation now', 'welowe' ) ?></a></div>
    ...
    <div class="form-content">
      ...
      <div class="form-content-inner clearfix">
        <div class="campaign-content-inner">
          ...
          <div class="desc"><?php echo welowe_limit_words( $excerpt_words, get_the_excerpt(), get_the_content() ); ?></div>
        </div>
        <div class="campaign-information clearfix">
          <div class="campaign-goal"> 
            <span class="c-label"><?php echo esc_html__('Goal: ', 'welowe') ?></span> 
            <span class="goal"><?php echo esc_html($goal) ?></span>
          </div>
          <div class="campaign-raised">
            <span class="c-label"><?php echo esc_html__('Raised: ', 'welowe') ?></span> 
            <span class="raised"><?php echo esc_html($income) ?></span>
          </div>
        </div>
        <div class="campaign-action"><a class="btn-give-theme" href="<?php esc_url(the_permalink()) ?>"><?php echo esc_html__('Read more', 'welowe') ?></a></div>
      </div>  
