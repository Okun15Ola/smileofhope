<?php
if(!function_exists('welowe_give_breadcrumb')){
	function welowe_give_breadcrumb(){
		$result = welowe_style_breadcrumb();
		extract($result);
	...
	add_action( 'welowe_give_before_main_content', 'welowe_give_breadcrumb', 20 );
}

function welowe_give_change_posts_per_page( $query ) {
	if ( is_admin() || ! $query->is_main_query() ) {
		return;
	}
	$posts_per_page = welowe_get_option('give_posts_per_page', 6);
	if ( is_post_type_archive( 'give_forms' ) ) {
		$query->set( 'posts_per_page', $posts_per_page );
	}
}
add_filter( 'pre_get_posts', 'welowe_give_change_posts_per_page' );

function welowe_give_get_donation_form_submit_button( $form_id, $args = array() ) {
  global $post;
  ...
  $display_label       = ( ! empty( $display_label_field ) ? $display_label_field : esc_html__( 'Donate Now', 'welowe' ) );
  ob_start();
  ?>
...
add_filter( '__give_donation_form_submit_button', 'welowe_give_get_donation_form_submit_button');

function welowe_give_display_checkout_button( $output ) {
  $output = '<div class="clearfix give-checkout-button">'.$output.'</div>';
  echo wp_kses( $output, true );
}

add_filter( '__give_display_checkout_button', 'welowe_give_display_checkout_button', 1);

add_action('give_embed_head', 'welowe_give_embed_head', 11);
function welowe_give_embed_head(){
