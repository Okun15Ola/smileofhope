...
class Welowe_Mega_Menu {
	...
	function menu_wp_edit_nav_menu_walker($class, $menu_id){
		return 'Welowe_Walker_Nav_Menu_Edit';
	}
...
class Welowe_Walker extends Walker {
	/**
 ...
 */
class Welowe_Walker_Nav_Menu_Edit extends Walker_Nav_Menu {
	/**
	 * Starts the list before the elements are added.
	 *
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( ! empty( $item->_invalid ) ) {
			...
			$title = sprintf( esc_html__( '%s (Invalid)' , 'welowe'), $item->title );
		} elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
			$classes[] = 'pending';
			/* translators: %s: title of menu item in draft status */
			$title = sprintf( esc_html__('%s (Pending)', 'welowe'), $item->title );
		}
		...
		<li id="menu-item-<?php echo esc_attr($item_id); ?>" class="<?php echo implode(' ', $classes ); ?>">
			<dl class="menu-item-bar">
               ...
               <span class="menu-megamenu-handle"><?php echo esc_html__('Mega Menu', 'welowe'); ?></span>
					<label class="item-title" for="menu-item-checkbox-<?php echo esc_attr($item_id); ?>">
						...
						<span class="is-submenu <?php echo esc_attr($submenu_text_class) ?>"><?php echo esc_html__('Sub item', 'welowe'); ?></span>
					</label>
					<span class="item-controls">
						...
						<span class="item-order hide-if-js">
							...
							?>" class="item-move-up"><abbr title="<?php echo esc_attr__('Move up', 'welowe'); ?>">&#8593;</abbr></a>
							|
							<a href="<?php
								echo wp_nonce_url(
									add_query_arg(
							...
							?>" class="item-move-down"><abbr title="<?php echo esc_attr__('Move down', 'welowe'); ?>">&#8595;</abbr></a>
						</span>
						<a class="item-edit" id="edit-<?php echo esc_attr($item_id); ?>" title="<?php echo esc_attr__('Edit Menu Item', 'welowe'); ?>" href="<?php
			...
			<div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr($item_id); ?>">
         	...
         	<div class="menu-megamenu" style="border: 2px solid #000;">
             	<h4 class="menu-megamenu-title"><?php echo esc_html__( 'Mega Menu Setting', 'welowe' ); ?></h4>
             	<p class="menu-megamenu-enable">
                     ...
                     <?php echo esc_html__( 'Mega Menu Enabled', 'welowe' ); ?>
                  </label>
               </p>
                  ...
                  <label>
                     <?php 
                      ...
                      echo esc_html__( 'Number columns', 'welowe' ); ?><br />
                  	...
                  	<p><?php  echo esc_html__( 'Number columns of submenu(support 1,2,3,4,6 columns)', 'welowe' ); ?></p>
                  </label>
              	</p>
                  ...
                  <label>
                     <?php if(empty($item->megawidth) || $item->megawidth==0){
                   	...
                   	echo esc_html__( 'Mega menu width', 'welowe' ); ?><br />
               ...
               <p class="menu-megamenu-megaalign">
                  <label>
                     ...
                     echo esc_html__( 'Mega menu Align', 'welowe' ); ?><br />
                     <select id="edit-menu-item-megaalign-<?php echo esc_attr($item_id); ?>" class="width-75 code edit-menu-item-megaalign" name="menu-item-megaalign[<?php echo esc_attr($item_id); ?>]">
                     	<option value="fullwidth" <?php echo esc_attr($item->megaalign == 'fullwidth' ? 'selected' : '') ?>><?php echo esc_html__( "Full Width", 'welowe' ) ?></option>
                     	<option value="center" <?php echo esc_attr($item->megaalign == 'center' ? 'selected' : '') ?>><?php echo esc_html__( "Center", 'welowe' ) ?></option>
                     	<option value="left" <?php echo esc_attr($item->megaalign == 'left' ? 'selected' : '') ?>><?php echo esc_html__( "Left", 'welowe' ) ?></option>
                     	<option value="right" <?php echo esc_attr($item->megaalign == 'right' ? 'selected' : '') ?>><?php echo esc_html__( "Right", 'welowe' ) ?></option>
                     </select>
                  </label>
              	</p>
              	<p class="menu-megamenu-submegamenu">
              		<label>
              			<?php echo esc_html__( 'Mega Menu', 'welowe' ); ?><br/>
                  	...
                  	<p><?php  echo esc_html__( 'The ID of Elementor Template Section', 'welowe' ); ?></p>
            		</label>
            	</p>
            </div>
            ...
            <?php // custom field for mega menu end ?>
             	<div>
                     ...
                     <label>
                         <?php echo esc_html__( 'Icon', 'welowe' ); ?> <a href="<?php echo is_ssl() ? 'https' : 'http'; ?>://fortawesome.github.io/Font-Awesome/icons/" target="_bank"><?php esc_html_e('(icons in Font Awesome)', 'welowe'); ?></a> <br />
				...
				<?php if( 'custom' == $item->type ) : ?>
					<p class="field-url description description-wide">
						<label for="edit-menu-item-url-<?php echo esc_attr($item_id); ?>">
							<?php echo esc_html__( 'URL' , 'welowe'); ?><br />
				...
				<p class="description description-thin">
					<label for="edit-menu-item-title-<?php echo esc_attr($item_id); ?>">
						<?php echo esc_html__( 'Navigation Label' , 'welowe'); ?><br />
						<input type="text" id="edit-menu-item-title-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
					</label>
				</p>
				<p class="description description-thin">
					<label for="edit-menu-item-attr-title-<?php echo esc_attr($item_id); ?>">
						<?php echo esc_html__( 'Title Attribute' , 'welowe'); ?><br />
				...
				<p class="field-link-target description">
					<label for="edit-menu-item-target-<?php echo esc_attr($item_id); ?>">
						...
						<?php echo esc_html__( 'Open link in a new window/tab' , 'welowe'); ?>
					</label>
				</p>
				<p class="field-css-classes description description-thin">
					<label for="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>">
						<?php echo esc_html__( 'CSS Classes (optional)' , 'welowe'); ?><br />
						<input type="text" id="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( implode(' ', $item->classes ) ); ?>" />
					</label>
				</p>
				<p class="field-xfn description description-thin">
					<label for="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>">
						<?php echo esc_html__( 'Link Relationship (XFN)' , 'welowe'); ?><br />
						<input type="text" id="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-xfn" name="menu-item-xfn[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->xfn ); ?>" />
					</label>
				</p>
				<p class="field-description description description-wide">
					<label for="edit-menu-item-description-<?php echo esc_attr($item_id); ?>">
						<?php echo esc_html__( 'Description' , 'welowe'); ?><br />
						<textarea id="edit-menu-item-description-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-description" rows="3" cols="20" name="menu-item-description[<?php echo esc_attr($item_id); ?>]"><?php echo esc_html( $item->description ); // textarea_escaped ?></textarea>
						<span class="description"><?php echo esc_html__('The description will be displayed in the menu if the current theme supports it.', 'welowe'); ?></span>
					</label>
				</p>

				<p class="field-move hide-if-no-js description description-wide">
					<label>
						<span><?php echo esc_html__( 'Move' , 'welowe'); ?></span>
						<a href="#" class="menus-move-up"><?php echo esc_html__( 'Up one' , 'welowe'); ?></a>
						<a href="#" class="menus-move-down"><?php echo esc_html__( 'Down one' , 'welowe'); ?></a>
						<a href="#" class="menus-move-left"></a>
						<a href="#" class="menus-move-right"></a>
						<a href="#" class="menus-move-top"><?php echo esc_html__( 'To the top' , 'welowe'); ?></a>
					</label>
				</p>
				...
				<div class="menu-item-actions description-wide submitbox">
					<?php if( 'custom' != $item->type && $original_title !== false ) : ?>
						<p class="link-to-original">
							<?php printf( esc_html__('Original: %s', 'welowe'), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
						</p>
					<?php endif; ?>
					...
					); ?>"><?php echo esc_html__( 'Remove' , 'welowe'); ?></a> <span class="meta-sep hide-if-no-js"> | </span> <a class="item-cancel submitcancel hide-if-no-js" id="cancel-<?php echo esc_attr($item_id); ?>" href="<?php echo esc_url( add_query_arg( array( 'edit-menu-item' => $item_id, 'cancel' => time() ), admin_url( 'nav-menus.php' ) ) );
						?>#menu-item-settings-<?php echo esc_attr($item_id); ?>"><?php esc_html_e('Cancel', 'welowe'); ?></a>
				</div>
...
$welowe_megamenu = new Welowe_Mega_Menu();
$welowe_megamenu->init_mega_menu();
