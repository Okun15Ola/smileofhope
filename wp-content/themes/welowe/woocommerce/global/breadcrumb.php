...
if ( ( ! is_front_page() && ! ( is_post_type_archive() && get_option( 'page_on_front' ) == wc_get_page_id( 'shop' ) ) ) || is_paged() ) {
	...
	if ( is_home() ) {
		...
		$output_html .= $prepend . $before . esc_html__( 'Products tagged &ldquo;', 'welowe' ) . $queried_object->name . '&rdquo;' . $after;
		...
		if ( is_search() ) {
			...
			$output_html .= $before . '<a href="' . get_post_type_archive_link( 'product' ) . '">' . $_name . '</a>' . $delimiter . esc_html__( 'Search results for &ldquo;', 'welowe' ) . get_search_query() . '&rdquo;' . $after;
		...
		$output_html .= $before . esc_html__( 'Error 404', 'welowe' ) . $after;
		...
		$output_html .= $before . esc_html__( 'Search results for &ldquo;', 'welowe' ) . get_search_query() . '&rdquo;' . $after;

	} elseif ( is_tag() ) {

			$output_html .= $before . esc_html__( 'Posts tagged &ldquo;', 'welowe' ) . single_tag_title( '', false ) . '&rdquo;' . $after;

	} elseif ( is_author() ) {

		$userdata = get_userdata( $author );
		$output_html .= $before . esc_html__( 'Author:', 'welowe' ) . ' ' . $userdata->display_name . $after;

	}

	if ( get_query_var( 'paged' ) ) {
		$output_html .= ' (' . esc_html__( 'Page', 'welowe' ) . ' ' . get_query_var( 'paged' ) . ')';
	}
